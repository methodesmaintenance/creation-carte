...
# Fonction pour nettoyer le texte (conservée pour d'éventuels besoins, mais plus appliquée à l'identifiant)
...
if uploaded_file is not None:
    if st.session_state.last_uploaded_file_name != uploaded_file.name:
        ...
        try:
            # LECTURE EN FORMAT TEXTE (dtype=str) pour préserver les identifiants (ex: zéros au début)
            if uploaded_file.name.endswith('.csv'):
                df_loaded = pd.read_csv(uploaded_file, dtype=str)
            else:
                df_loaded = pd.read_excel(uploaded_file, dtype=str)
            
            # On ne nettoie plus toutes les colonnes par défaut pour éviter d'altérer l'identifiant !
...
if st.session_state.df_geocoded is not None:
    ...
    if st.button("🗺️ Générer la carte des secteurs"):
        with st.spinner("Calcul des secteurs et génération de la carte..."):
            ...
            # 1. Préparation de la colonne Secteur au format texte lisible
            if clustering_mode == "Sectorisation intelligente":
                df_ready['Secteur'] = df_ready['cluster'].apply(lambda x: f"Secteur {int(x) + 1}")
            elif clustering_mode == "Regroupement par colonne":
                reverse_mapping = {v: k for k, v in cluster_mapping.items()}
                df_ready['Secteur'] = df_ready['cluster'].map(reverse_mapping)
            elif use_agency_clustering:
                agency_names = st.session_state.agences_df['Name'].to_dict()
